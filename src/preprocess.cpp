...
void Preprocess::robosense_handler(const sensor_msgs::PointCloud2::ConstPtr& msg) {
  ...
  double msg_stamp = ros::Time(msg->header.stamp).toSec();
  pcl::PointCloud<robosense_ros::Point> pl_orig, pl_raw;
  pcl::fromROSMsg(*msg, pl_raw);

  // FAST_LIO is not able to deal with NaN / non-finite values. Clean nan/inf points
  for (const auto& pt : pl_raw.points)
    if (std::isfinite(pt.x) and std::isfinite(pt.y) and std::isfinite(pt.z))
      pl_orig.points.push_back(pt);
  ...
  if (feature_enabled) {
    ...
    for (uint i = 0; i < plsize; i++) {
      ...
      // Adding the time as a difference from the header stamp, instead of absolute time
      added_pt.curvature = (pl_orig.points[i].timestamp - msg_stamp) * time_unit_scale;
